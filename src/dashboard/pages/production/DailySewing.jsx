...
const sampleData = [
    ...
    {
        id: 4, date: '2026-08-30', lineNumber: 'L-03', jobNumber: 'JOB-0998', styleNumber: 'STY-D02', color: 'Grey',
    ...
    return (
        <div className="w-full rounded-lg border border-gray-200 bg-white shadow-sm">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 bg-white px-3 py-2">
                <h3 className="text-sm font-semibold text-slate-800">Daily Sewing Report</h3>
                    ...
                    <thead>
                        <tr className="bg-slate-800 text-white">
                            {LEFT_COLUMNS.map((col) => (
                                <th key={col.key} rowSpan={2} className="border border-slate-700 px-3 py-2 text-center font-semibold align-middle whitespace-nowrap">
                                    {col.label}
                                    <FilterDropdown
                                        label={col.label}
                            ...
                            <th colSpan={HOUR_COLUMNS.length} className="border border-slate-700 px-3 py-2 text-center font-semibold">
                                HOUR & PRODUCTION
                            </th>
                            <th rowSpan={2} className="border border-slate-700 px-3 py-2 text-center font-semibold align-middle whitespace-nowrap">
                                {REMARKS_COLUMN.label}
                                <FilterDropdown
                                    label={REMARKS_COLUMN.label}
                                    options={getUniqueValues(REMARKS_COLUMN.key)}
                                    selected={filters[REMARKS_COLUMN.key]}
                                    onToggle={(val) => toggleFilterValue(REMARKS_COLUMN.key, val)}
                                    onClear={() => clearFilter(REMARKS_COLUMN.key)}
                                />
                            </th>
                        </tr>
                        <tr className="bg-slate-700 text-white">
                            {HOUR_COLUMNS.map((col) => (
                                <th key={col.key} className="border border-slate-600 px-2 py-2 text-center font-medium whitespace-nowrap">
                                    {col.label}
                                    <FilterDropdown
                                        label={col.label}
                                        options={getUniqueValues(col.key)}
