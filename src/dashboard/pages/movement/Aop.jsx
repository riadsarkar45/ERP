...
const Aop = () => {
    ...
    const allRows = useMemo(() => {
        ...
        // FIX: Group by Challan + JobNo + Composition so each job gets its own row
        ...
        movements.forEach((item) => {
            ...
            if (Array.isArray(item.deliveries) && item.deliveries.length > 0) {
                // NESTED shape: Each movement has exactly ONE jobNo and ONE composition
                ...
                item.deliveries.forEach((dv) => {
                    ...
                    // Unique key ensures separate rows for different jobs on the same challan
