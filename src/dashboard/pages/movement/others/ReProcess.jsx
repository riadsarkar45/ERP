...
const tableHeader = [
    ...
    { header: "Sent Reprocess", width: "7%", key: "sentForReprocess" },
    { header: "Recv Reprocess", width: "7%", key: "receivedFromReprocess" },
...
const ReProcess = () => {
    ...
    useEffect(() => {
        if (search) return;
        fetchData(`/api/challan-movement/dyeingOrder/reprocess?page=${page}&limit=10`)
            .then(data => {
                if (data) {
    ...
    const allRows = useMemo(() => {
        ...
        const applyDelivery = (row, dv, source) => {
            ...
            if (type.includes("sentforreprocess") || type.includes("senttoreprocess")) row.sentForReprocess += qty;
            else if (type.includes("receivedfromreprocess") || type.includes("receivefromreprocess")) row.receivedFromReprocess += qty;
            else if (type.includes("finishreceive") || type.includes("finishreceived") || type.includes("returnfromreprocess") || type.includes("finishfromreprocess")) row.finishReceive += qty;
                    ...
                    <tbody>
                        {filteredRows.map((row) => (
                            <tr key={row.rowKey}>
                                ...
                                <td style={cellStyle}>{row.sentForReprocess > 0 ? row.sentForReprocess : "-"}</td>
                                <td style={cellStyle}>{row.receivedFromReprocess > 0 ? row.receivedFromReprocess : "-"}</td>
