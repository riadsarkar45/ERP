...
const RequestedData = ({ byUser, requestedAt, handleUserOnchange, workOrder, handleDownloadPdf, downloadError, isDownloading, concernPersons }) => {
            ...
            <div className={`flex pb-2 items-center justify-between px-4 py-2 border-b border-gray-200 rounded-t-lg`}>
                ...
                <div className="flex gap-2 items-center">
                    <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">
                        <button
                            type="button"
                            onClick={() => handleDownloadPdf(workOrder?.id, workOrder?.jobNo)}
                            disabled={isDownloading || !workOrder?.id}
                            title={!workOrder?.id ? 'Missing work order id' : 'Download PDF'}
                    ...
                    {
                        workOrder?.isRequested === true &&
                        <div>
                            <select onChange={(e) => handleUserOnchange(e, workOrder?.id)}>
