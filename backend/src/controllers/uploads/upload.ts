    ...
    try {
        ...
        const workOrders = cleanData
            ...
            .map((row) => ({
                ...
                salesContractNo      : row["SALES CONTRACT NO."] ?? null,
                bookingColor         : row["BOOKING COLOR"] ?? null,
                composition          : row["COMPOSITION"] ?? null,
                dia                  : row["DIA"]?.toString() ?? null,
                yarnCount            : row["YARN COUNT"]?.toString() ?? null,
                brandLot             : row["BRAND / LOT"]?.toString() ?? null,
                yarnDyeingColor      : row["YARN DYEING COLOR"] ?? null,
                yarnDyedWorkOrderQty : row[" YARN DYED WORK ORDER (QTY) "] ?? null,
                yarnDeliveryForYD    : row[" YARN DELIVERY FOR Y/D "] ?? null,
                delShortExcess       : row["DEL. SHORT & EXCESS"] ?? null,
                yarnReturnReceived   : row["YARN RETURN RECEIVED"] ?? null,
                greyReceivedFromYD   : row["GREY RECEIVED FROM Y/D"] ?? null,
                finishYarnReceived   : row["FINISH YARN RECEIVED"] ?? null,
                yarnStock            : row["YARN STOCK"] ?? null,
                ydProcessLoss        : row["Y/D PROCESS LOSS AS PER BOOKING"] ?? null,
                processLossAfterYD   : row["PROCESS LOSS AFTER Y/D"] ?? null,
                ydPricePerKg         : row["Y/D PRICE PER KG"] ?? null,
                totalBillingAmount   : row[" TOTAL BILLING AMOUNT "] ?? null,
                deductionForDebitNote: row[" DEDUCTION FOR DEBIT NOTE "] ?? null,
                payableAmount        : row[" PAYABLE AMOUNT "] ?? null,
                billNo               : row[" BILL NO. "]?.toString() ?? null,
                paidBillingAmount    : row[" PAID BILLING AMOUNT "] ?? null,
                pendingBillingAmount : row[" PENDING BILLING AMOUNT "] ?? null,
